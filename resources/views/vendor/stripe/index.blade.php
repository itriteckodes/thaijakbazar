        ...
        <div class="row mt-5">
            <div class="col-md-6 col-md-offset-3">
                <div class="panel panel-default credit-card-box">
                    ...
                    <form role="form" action="{{route('user.payment_stripe')}}" method="post" class="require-validation"
                            data-cc-on-file="false" data-stripe-publishable-key="{{ "pk_test_51GxWFUBWndAApmnXy1V3eBe1Up1u3DpEe18J1YQTXzH3lX6aGqp2x37wbW9490pxcOEonsT68gn9fAIgk7z13A7Z00JdJA6Dqj" }}"
                            id="payment-form">
                            @csrf
                            ...
                            <div class='form-row row'>
                                ...
                                <div class='col-xs-12 col-md-4 form-group expiration required'>
                                    <label class='control-label'>{{__('Expiration Month')}}</label> <input
                                        class='form-control card-expiry-month' placeholder='{{__('MM')}}' size='2' type='text'>
                                </div>
                                <div class='col-xs-12 col-md-4 form-group expiration required'>
                                    <label class='control-label'>{{__('Expiration Year')}}</label> <input
                                        class='form-control card-expiry-year' placeholder='{{__('YYYY')}}' size='4' type='text'>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-xs-12">
                                    <button class="btn btn-primary btn-lg btn-block" type="submit">{{__('Pay Now')}}
...
<script type="text/javascript">
  ...
  function stripeResponseHandler(status, response) {
        if (response.error) {
            ...
            $form.append("<input type='hidden' name='package_id' value='{{$package->id}}'/>");
            $form.get(0).submit();
        }
    }
