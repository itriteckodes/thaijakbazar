                    ...
                    @foreach (App\Models\DepositMethod::active() as $key=> $method)
                        <div class="form-check">
                            <label class="form-check-label">
                                <input type="radio" class="form-check-input-styled txt4" name="payment" value="{{$method->handle}}"  data-fouc {{$key == '0' ? 'checked':''}}>
                                {{$method->name}}
                            </label>
                        </div>
                    @endforeach
            ...
            <div class="row">
                <div class="col-md-12 col-md-offset-3">
                    <div class="panel panel-default credit-card-box">
                        ...
                        <form role="form" action="{{route('user.deposit.store')}}" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ $cardpayment->publishable_key }}" id="payment-form">
                        @csrf
                        ...
                        <div class='row'>
                            ...
                            <div class="col-md-12 form-group">
                                <div class="col-xs-12 txt4">
                                    <label>{{__('Amount')}}</label> 
                                    <input class='form-control txt4' placeholder='{{__('Enter Amount')}}' size='4' type='number' name="amount" required>
                                </div>
                            </div>
                            <div class="col-md-12 form-group">
                                <div class="col-xs-12 txt4">
...
<div class="row desc" id="jazzcash">
    ...
    <div class="col-md-6 col-md-offset-3">
        <div class="card">
            ...
            {{-- <div class="card-body">
                <p class="card-text txt4">Example of the card image, placed right after card header and before card content body. Image requires <code>.img-fluid</code> class for proper sizing.</p>
            </div> --}}
            <form class="order-form" action="{{route('user.deposit.store')}}" method="POST">
                @csrf
                <div class="card-footer">
                    <div class="input-group">
                        <input type="text" name="amount" class="form-control border-right-0 txt4" placeholder="{{__('Enter Amount')}}" required>
                        <input type="hidden" name="gateway_id" value="{{gateway()->jazzCash()}}">
                        <span class="input-group-append txt4">
                            <button class="btn bg-blue" type="submit"><i class="icon-paperplane"></i></button>
                        </span>
                    </div>
                </div>
            </form>
...
<div class="row desc" id="jakcoin">
    ...
    <div class="col-md-6 col-md-offset-3">
        <div class="card">
            <div class="card-header bg-transparent">
                <h6 class="card-title text-center txt4">Jak Coins</h6>
